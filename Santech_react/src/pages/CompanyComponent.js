...
// Ce composant gère les opérations sur les entreprises, y compris leur création et affichage.
class CompanyComponent extends React.Component {
  constructor(props) {
    super(props);
    this.formSubmit = this.formSubmit.bind(this); // Liaison de la méthode formSubmit pour utiliser 'this' correctement.
  }
  state = {
    errorRes: false, // Pour afficher les erreurs de formulaire
    errorMessage: "", // Message d'erreur à afficher
    btnMessage: 0, // Contrôle le message affiché sur le bouton pendant l'envoi du formulaire
    sendData: false, // Indique si les données ont été envoyées
    companyDataList: [], // Liste des données des entreprises
  ...
  // Appelé après que le composant a été monté, pour charger les données initiales.
  componentDidMount() {
  ...
  async fetchCompanyData() {
    ...
    console.log(companydata); // Pour le débogage
    this.setState({ companyDataList: companydata.data.data, dataLoaded: true });
  }

  // Redirige vers la page de détails d'une entreprise spécifique.
