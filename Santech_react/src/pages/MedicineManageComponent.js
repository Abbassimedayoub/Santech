...
class MedicineManageComponent extends React.Component {
  constructor(props) {
    super(props);
    this.formSubmit = this.formSubmit.bind(this); // Binds the formSubmit function to the component instance.
  }
  state = {
  ...
  async formSubmit(event) {
    ...
    this.setState({ btnMessage: 1 }); // Indicates loading or processing state.
    var apiHandler = new APIHandler();
    var response = await apiHandler.editMedicineData(
      // Call to API to edit medicine data using the form values.
    ...
    this.setState({ btnMessage: 0 }); // Reset button message state after API call.
    this.setState({ errorRes: response.data.error }); // Update error state based on API response.
    this.setState({ errorMessage: response.data.message }); // Update error message.
    this.setState({ sendData: true }); // Flag to indicate that data has been sent.
  }

  componentDidMount() {
    this.LoadInitialData(); // Load initial data when the component mounts.
  }

  async LoadInitialData() {
    var apihandler = new APIHandler();
    var companydata = await apihandler.fetchCompanyOnly(); // Fetch company data.
    var medicinedata = await apihandler.fetchAllMedicine(); // Fetch all medicine data.
    this.setState({ companylist: companydata.data }); // Set company list in state.
    this.setState({ medicineDataList: medicinedata.data.data }); // Set medicine data list in state.
    this.setState({ dataLoaded: true }); // Indicate that initial data has been loaded.
  }

  RemoveItems = () => {
    // Function to remove an item from the medicinedetails array.
    if (this.state.medicinedetails.length != this.state.total_salt_list) {
      this.state.medicinedetails.pop(this.state.medicinedetails.length - 1); // Remove the last item.
    }
    this.setState({});
  };

  handleInput = (event) => {
    // Handle input changes for the medicinedetails fields.
    var keyname = event.target.name;
    var value = event.target.value;
    var index = event.target.getAttribute("data-index");
    this.state.medicinedetails[index][keyname] = value; // Update specific detail in the array.
    this.setState({});
    console.log(this.state.medicinedetails);
  };

  AddItem = () => {
    // Function to add a new item to the medicinedetails array.
    var item = {
      salt_name: "",
      salt_qty: "",
      salt_qty_type: "",
    ...
    this.state.medicinedetails.push(item); // Add new item to the array.
    this.setState({});
  };

  viewmedicineDetails = (index) => {
    // Function to load and view details of a selected medicine.
    console.log(this.state.medicineDataList[index]);
    this.setState({ medicine_id: this.state.medicineDataList[index].id }); // Set selected medicine id.
    this.setState({ name: this.state.medicineDataList[index].name }); // Update state with selected medicine details.
    this.setState({
    ...
    this.setState({ TVA: this.state.medicineDataList[index].TVA });
              ...
              <div className="card">
                ...
                <div className="body">
                  <form onSubmit={this.formSubmit}>
                    ...
                    <label htmlFor="email_address">TVA</label>
                    <div className="form-group">
                      <div className="form-line">
                        <input
                          type="text"
